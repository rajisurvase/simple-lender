...
import {
  AddBorrowerSchema,
  AddEditBorrowerType,
} from "@/schema/borrower.schema";
import dayjs from "dayjs";
import CustomDatePicker from "@/ui/DatePicker/CustomDatePicker";

type BorrowersCreateProps = {
  handleClose: () => void;
  selectedBorrower: AddEditBorrowerType | undefined;
  refetch: () => void;
};
const BorrowersCreate = ({
  handleClose,
  selectedBorrower,
  refetch,
}: BorrowersCreateProps) => {
  const { toastSuccess, toastError } = useNotiStack();
  const { register, control, handleSubmit, reset } =
    useForm<AddEditBorrowerType>({
      resolver: yupResolver(AddBorrowerSchema),
      defaultValues: {
        first_name: selectedBorrower?.first_name || "",
        last_name: selectedBorrower?.last_name ?? "",
  ...
  const onSubmit = handleSubmit((data) => {
    if (selectedBorrower?._id) {
      editBorrowerMutation({ ...data, _id: selectedBorrower?._id });
    } else {
      AddNewBorrower(data);
    }
  });
